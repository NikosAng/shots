...
for(league in leagues){
  ...
  for(url in urls[[league]]){
    ...
    pass_layoff_goals <- sum((data$lastAction == "Cross")&data$situation=='OpenPlay' & data$result == "Goal", na.rm = TRUE)
    total_goals <- sum(data$result == "Goal", na.rm = TRUE)
...
# Write data to CSV
write_csv(pass_layoff_goals_percentage, "cross_goals_percentage.csv")
